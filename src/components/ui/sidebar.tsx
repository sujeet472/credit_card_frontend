...
// console.log("isMobile ", isMobile, "openMobile ", openMobile);
  ...
  if(isMobile && openMobile){
    return (
      <Button
        ...
        variant="ghost"
        size="icon"
        className= {cn("h-7 w-7 invisible", className )}
        onClick={(event) => {
          onClick?.(event)
          toggleSidebar()
        }}
...
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn("flex flex-col gap-2 p-2 bg-white", className)}
      {...props}
    />
  )
}
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 bg-white overflow-auto group-data-[collapsible=icon]:overflow-hidden",
        className
      )}
      {...props}
    />
